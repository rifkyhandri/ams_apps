...
<!-- Tittle -->
<div class="row">
    @section('condition_user_id'){{Auth::user()->id}}@stop
    @section('modal_import_excel'){{route('condition.import')}}@stop
        ...
        <div style="height: 65px; border-radius: 15px;" class="card">
            <div class="card-body">
                <div style="display: flex; align-items: flex-start; justify-content: space-between; height: 40px;">
                    <h4 class="card-title" style="font-size: 2rem; font-weight: 300; margin-bottom: 0px;">Condition</h4>
                    ...
                    <button style="margin: -0.2rem .05rem -1rem auto;" type="button" class="btn btn-primary btn-sm"
                        data-toggle="modal" data-target="#showModalInsertCondition"><i
...
<!-- Tittle End -->
<a href="{{route('condition_export')}}" class="btn btn-primary btn-sm btn-rounded-sm mb-3 mt-3">Export Excel</a>
                ...
                <div class="row">
                    ...
                    <div class="table-responsive">
                        ...
                        <thead>
                            <tr>
                                <th>No</th>
                                <th style="width: 100px;">Condition Code</th>
...
<script>
    ...
    $(document).ready(function () {
        //RENDER DATA TABLES
        mTable = $('#conditionTable').DataTable({
            responsive: true,
            processing: true,
            "language": {
            ...
            columns: [
                ...
                },
                {
                    ...
                    render: function (data, type, row) {
                        let buttonEdit =
                            '<button type="button" class="btn btn-success btn-rounded btn-icon" data-toggle="modal" data-placement="buttom" data-custom-class="tooltip-success" title="EDIT" data-target="#showModalUpdateCondition" style="margin-right:5px;" onclick="buttonEdit(\'' + data + '\');"><i style="font-size:1.5rem; margin-left:-7px;" class="ti-pencil-alt"></i></button>';
                        let buttonHapus =
                            ...
                            data +'\',\'condition\');"><i style="font-size:1.5rem; margin-left:-8px;" class="ti-trash"></i></button>';
        ...
        //INSERT Departement
        $('#formInputCondition').on('submit', function (e) {
            e.preventDefault();

            var url,data,form;

            url = 'condition';           //For routing
            data = $(this).serialize();    // Data Form
            form = 'Condition';          //Reset Trigger and Close Modal
        ...
        //UPDATE Departement
        $('#formUpdateCondition').on('submit', function (e) {
    ...
    function buttonEdit(id) {
        ...
        $.ajax({
            type: "GET",
            url: "{{ route('condition.edit',['condition'=>1]) }}",
            data: {
                id: id
            },
            success: function (data) {
                if (data) {
                    clear_error();
                    // console.log(data);
                    $("#idCondition").val(data.id);
                    $("#Updateconditioncode").val(data.conditioncode);
                    $("#Updateconditiondesc").val(data.conditiondesc);
                } else {
