...
<!-- Tittle -->
<div class="row">
    @section('assettype_user_id'){{Auth::user()->id}}@stop
    @section('modal_import_excel'){{route('assettype.import')}}@stop
        ...
        <div style="height: 65px; border-radius: 15px;" class="card">
            <div class="card-body">
                <div style="display: flex; align-items: flex-start; justify-content: space-between; height: 40px;">
                    <h4 class="card-title" style="font-size: 2rem; font-weight: 300; margin-bottom: 0px;">Asset Type</h4>
                    ...
                    <button style="margin: -0.2rem .05rem -1rem auto;" type="button" class="btn btn-primary btn-sm"
                        data-toggle="modal" data-target="#showModalInsertAssetType"><i
...
<!-- Tittle End -->
<a href="{{route('assettype_export')}}" class="btn btn-primary btn-sm btn-rounded-sm mb-3 mt-3">Export Excel</a>
        ...
        <div class="card" style="border-radius: 15px;">
                ...
                <div class="row">
                    ...
                    <div class="table-responsive">
                        <table id="assettypeTable" class="table">
                        <thead>
                            <tr>
                                <th>No</th>
...
<script>
    ...
    $(document).ready(function () {
        //RENDER DATA TABLES
        mTable = $('#assettypeTable').DataTable({
            responsive: true,
            processing: true,
            "language": {
            ...
            columns: [
                ...
                },
                {
                    ...
                    render: function (data, type, row) {
                        let buttonEdit =
                            '<button type="button" class="btn btn-success btn-rounded btn-icon" data-toggle="modal" data-placement="buttom" data-custom-class="tooltip-success" title="EDIT" data-target="#showModalUpdateAssetType" style="margin-right:5px;" onclick="buttonEdit(\'' + data + '\');"><i style="font-size:1.5rem; margin-left:-7px;" class="ti-pencil-alt"></i></button>';
                        let buttonHapus =
                            ...
                            data +'\',\'assettype\');"><i style="font-size:1.5rem; margin-left:-8px;" class="ti-trash"></i></button>';
        ...
        //INSERT Departement
        $('#formInputAssetType').on('submit', function (e) {
            e.preventDefault();

            var url,data,form;

            url = 'assettype';           //For routing
            data = $(this).serialize();    // Data Form
            form = 'AssetType';          //Reset Trigger and Close Modal
        ...
        //UPDATE Departement
        $('#formUpdateAssetType').on('submit', function (e) {
            e.preventDefault();
    ...
    function buttonEdit(id) {
        ...
        $.ajax({
            type: "GET",
            url: "{{ route('assettype.edit',['assettype'=>1]) }}",
            data: {
                id: id
            },
            success: function (data) {
                if (data) {
                    clear_error();
                    console.log(data);
                    $("#idAssetType").val(data.id);
                    $("#Updatetypecode").val(data.id_typeasset);
                    $("#Updatetypedesc").val(data.description);
                } else {
