...
<h6 class="font-weight-bold text-danger">Revalue Report</h6>
...
<h6 class="font-weight-bold text-danger">PT Wahana Datarindo Sempurna</h6>
    <table class="table table-sm table-bordered table-striped ">
        ...
        <tbody style="text-align: center" >
            @foreach ($revalue_report as $e => $revalue)    
                <tr>
                    <td>{{$e+1}}</td>
                    <td>{{$revalue->tangnumber}}</td>
                    <td>{{$revalue->approveasset->serial}}</td>
                    <td>{{$revalue->approveasset->notes}}</td>
                    <td>{{$revalue->approveasset->models}}</td>
                    <td>{{$revalue->approveasset->account}}</td>
                    <td>{{$revalue->transaction_date}}</td>
                    <td>{{$revalue->wd_value}}</td>
                    <td>{{$revalue->extend_year}}</td>
                </tr>
            @endforeach
        </tbody>
    </table>
