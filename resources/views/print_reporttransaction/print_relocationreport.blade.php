...
<h6 class="font-weight-bold text-danger">Relocation Report</h6>
...
<h6 class="font-weight-bold text-danger">PT Wahana Datarindo Sempurna</h6>
    <table class="table table-sm table-bordered ">
        ...
        <tbody style="text-align: center" >
            @foreach ($relocation_report as $e => $relocation)    
                <tr>
                    <td>{{$e+1}}</td>
                    <td>{{$relocation->tangnumber}}</td>
                    <td>{{$relocation->approveasset->serial}}</td>
                    <td>{{$relocation->approveasset->notes}}</td>
                    <td>{{$relocation->approveasset->models}}</td>
                    <td>{{$relocation->approvelocation->location_sub->locationname_sub}}||{{$relocation->approvelocation->locationname_sm}}</td>
                    <td>{{$relocation->approvecostcenter->costcenterdesc}}</td>
                    <td>{{$relocation->approvecustodian->custodianname}}</td>
                </tr>
            @endforeach
        </tbody>
    </table>
